	       ...
	       struct rte_pci_device *pci_dev)
{
	...
	if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
		...
		rte_atomic32_add(&mana_shared_data->secondary_cnt, 1);
...
mana_pci_remove(struct rte_pci_device *pci_dev)
{
	...
	if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
		...
		RTE_VERIFY(rte_atomic32_read(&mana_shared_data->secondary_cnt)
				> 0);
		rte_atomic32_sub(&mana_shared_data->secondary_cnt, 1);
